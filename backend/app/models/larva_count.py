...
class LarvaCount(Base):
    __tablename__ = "larva_counts"
    __table_args__ = (UniqueConstraint("pond_id", "count_date", name="uq_pond_count_date"),)

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    pond_id: Mapped[int] = mapped_column(ForeignKey("ponds.id"), nullable=False, index=True)
    # 盘点日按东八区日历日切分，由前端以 UTC+8 日历日提交
    ...
    pond: Mapped["Pond"] = relationship("Pond", back_populates="larva_counts")
